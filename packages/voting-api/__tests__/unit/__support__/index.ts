...
import { Handlers as VotingHandlers, Indexers } from "@protokol/voting-transactions";
...
export const buildWallet = (app: Application, passphrase: string): Contracts.State.Wallet => {
	...
	wallet.setBalance(Utils.BigNumber.make(10000000000));
...
export const initApp = (): Application => {
	...
	app.bind<Contracts.State.WalletIndexerIndex>(Container.Identifiers.WalletRepositoryIndexerIndex).toConstantValue({
		name: Indexers.createProposalVotingWalletIndex,
		indexer: () => {},
		autoIndex: false,
	});

	app.bind<Contracts.State.WalletIndexerIndex>(Container.Identifiers.WalletRepositoryIndexerIndex).toConstantValue({
		name: Indexers.castVoteVotingWalletIndex,
		indexer: () => {},
		autoIndex: false,
	});
