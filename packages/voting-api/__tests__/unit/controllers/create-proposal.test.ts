...
import { CreateProposalController } from "../../../src/controllers";
import { blockHistoryService, initApp, ItemResponse, transactionHistoryService } from "../__support__";

let app: Application;

let createProposalController: CreateProposalController;

beforeAll(() => {
	app = initApp();

	createProposalController = app.resolve<CreateProposalController>(CreateProposalController);
});
describe("Create Proposal Controller", () => {
	...
	describe("Should Test Wallet Method", () => {
		it("Should Return Correct Response", async () => {
			...
			walletRepository.setOnIndex(Indexers.createProposalVotingWalletIndex, "senderPublicKeyID", {
				empty: "wallet",
